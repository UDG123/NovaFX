"""
NovaFX Signal Optimizer - Multi-timeframe RSI + MACD confluence.
...
logger = logging.getLogger("novafx.forex.optimizer")
...
async def multi_timeframe_signal(symbol: str) -> dict | None:
    """
    Multi-timeframe RSI + MACD confluence.
    ...
    # Convert symbol format for TwelveData: EURUSD -> EUR/USD
    td_symbol = symbol
    ...
    closes_1h, closes_4h = await asyncio.gather(
        fetch_candles(td_symbol, "1h", 100),
        fetch_candles(td_symbol, "4h", 50),
    )
    ...
    if rsi_1h < 38 and rsi_4h > 42 and macd_hist > -0.0005:
        signal = "BUY"
    ...
    elif rsi_1h > 62 and rsi_4h < 58 and macd_hist < 0.0005:
        signal = "SELL"
    ...
    # ATR-based SL/TP (more precise than fixed pips)
    sl_dist = atr * 1.5  # 1.5x ATR stop loss
    tp_dist = atr * 3.0  # 3.0x ATR take profit (2:1 ratio)
